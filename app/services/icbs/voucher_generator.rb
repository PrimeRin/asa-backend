module Icbs
  class VoucherGenerator
    def self.generate_voucher(txn_date:, txn_value_date:, particulars:, vch_type:, created_by:, amount:, dr_gl_code:, cr_gl_code:, emp_code:, monthly_recovery_amount:, from_date:, to_date:, loan_id:)
      oracle_formatted_txn_date = txn_date.strftime('%d-%b-%Y').upcase
      oracle_formatted_value_date = txn_value_date.strftime('%d-%b-%Y').upcase
      oracle_formatted_from_date = from_date.strftime('%d-%b-%Y').upcase
      oracle_formatted_to_date = to_date.strftime('%d-%b-%Y').upcase
      next_txn_id = nil
        ...
        sql = <<-SQL
          ...
          BEGIN
            ASA_GENERATE_VOUCHER(
              ...
              TO_DATE('#{oracle_formatted_from_date}', 'DD-MON-YYYY'),
              TO_DATE('#{oracle_formatted_to_date}', 'DD-MON-YYYY'),
              #{loan_id},
              v_next_txn_id
            );
